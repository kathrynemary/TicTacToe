...
describe Game do
  ...
  #   expect(@x.player1.symbol("X")).to eq(Player.new.symbol("X"))
  # end
  ...
  it "should ask for each player's symbol" do
    allow(@x.choose_symbol(@player1)).to receive(:gets).and_return("X")
    expect(@x.player1.symbol).to eq(Player.new.symbol("X"))
  end
